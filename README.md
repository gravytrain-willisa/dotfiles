...
**Using JRebel from an IntelliJ WSL run target** — IntelliJ's WSL JVM launch
execs `java` straight via `wsl.exe -e`, the same PAM-bypassing path as its
git integration above (confirmed empirically: `wsl.exe -d <distro> -e
printenv JAVA_TOOL_OPTIONS` comes back empty even with a shell export or an
`/etc/environment` entry in place), so nothing chezmoi manages —
`JREBEL_HOME`, or any other shell/`/etc/environment` export — ever reaches
it. Unlike git (`core.sshCommand`/`gpg.ssh.program`), there's no config point
to redirect through a wrapper: IntelliJ resolves the JDK to an absolute path
(typically through SDKMAN's `candidates/java/current/bin/java` symlink
chain) and execs that directly, so a wrapper would mean physically replacing
the resolved `java` binary itself, which `sdk install`/`sdk upgrade` would
silently overwrite or bypass — not worth the fragility for one IDE's launch
path. Set `JAVA_TOOL_OPTIONS` directly on the Run/Debug Configuration (or the
WSL run target) instead — IntelliJ injects environment variables into the
process it spawns directly, regardless of `wsl.exe -e` bypassing PAM. Example
(from a real working setup):
```
JAVA_TOOL_OPTIONS=-agentpath:/home/<user>/.local/share/jrebel/lib/libjrebel64.so -Drebel.projectpath.<project-id>.<module-name>=/home/<user>/projects/<project> -Drebel.license.email='you@example.com' -Drebel.license.url='https://licenses.zeroturnaround.com/<your-license-id>'
```
- `-agentpath:...` — path to the installed `.so`; `$JREBEL_HOME` (exported by
  `dot_zshrc.tmpl` as `~/.local/share/jrebel`) has to be written out in full
  here since IntelliJ's field doesn't expand shell variables. Confirm the
  actual path first: `ls $JREBEL_HOME/lib/libjrebel64.so`. If it's still
  nested one level deeper, at `$JREBEL_HOME/jrebel/lib/...`, that's a leftover
  from before `update-jrebel` started clearing `$JREBEL_HOME` on every
  refresh (see `.chezmoitemplates/jrebel-install.sh.tmpl`) — running
  `update-jrebel` once more cleans it up and moves the `.so` to
  `$JREBEL_HOME/lib/...`, so update the IntelliJ run config's `-agentpath` to
  match afterward.
- `-Drebel.projectpath.<project-id>.<module-name>=<path>` — project-specific:
  `<project-id>` is generated by JRebel per project, `<module-name>` matches
  the module name, `<path>` is the project's root inside WSL.
- `-Drebel.license.email` / `-Drebel.license.url` — user-specific, from your
  own JRebel license — never commit real values for these anywhere.

None of this can be templated by chezmoi (it's IDE-config, not a dotfile, and
two of the four values are inherently per-person/per-project), so it's set
by hand once per Run Configuration. The official JRebel IntelliJ plugin is
an alternative to hand-rolling `-D` flags like this — it manages the
equivalent settings through its own IDE UI instead.
