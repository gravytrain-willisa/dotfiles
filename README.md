...
| `gh auth login` | Runs automatically on every `chezmoi apply`, but is itself an interactive OAuth device-flow you approve in a browser when authentication is needed — or map a `GH_TOKEN` in `op-env-vars.txt` to skip the prompt entirely (item 25, "gh auth login") |
...
| Brew packages | [`dot_config/dotfiles/brew-packages.txt`](dot_config/dotfiles/brew-packages.txt) |
...
Install scripts, in execution order (see docs/setup-sequence.md)
  ...
  run_after_0019-gh-auth-login.sh.tmpl
  run_once_0020-configure-powershell-profile.sh.tmpl
```
