...
0. **Passwordless sudo for the bootstrap commands (Linux only)** — runs
   ...
   sudo — `apt-get update`, `/usr/local/sbin/dotfiles-append-shell *`,
   `/usr/local/sbin/dotfiles-append-environment *`, `chsh -s * <user>`, and
   — deliberately *not* a `apt-get install -y *` wildcard — one exact-match
   line per specific `apt-get install -y ...` invocation actually used
   below: the shell package (step 1), Homebrew's apt prerequisites (3),
   `unzip zip` for SDKMAN (7), pyenv's build deps (11), and `socat` (15). A
   wildcard there would let anything holding this rule install an arbitrary
   package for the file's whole lifetime — a root-run maintainer script is
   as good as NOPASSWD ALL — so each install this repo actually needs is
   spelled out instead; anything else still prompts normally, and a script
   whose package list drifts out of sync with this file just costs an extra
   password prompt, not a security regression.

   The two `dotfiles-append-*` entries are wildcarded on their argument, but
   that's safe here specifically because they aren't `tee`: this same step
   also installs `/usr/local/sbin/dotfiles-append-shell` and
   `/usr/local/sbin/dotfiles-append-environment` as small root-owned (not
   user-writable) scripts that validate the argument's *shape* — an
   executable path under a known shell-install directory, or a literal
   `SSH_AUTH_SOCK=...` line — before appending anything, refusing everything
   else. A bare `sudo tee -a /etc/shells`/`/etc/environment` would be an
   exact command/argument match too, but sudoers can't restrict what's
   piped into a command's stdin, only the command and its arguments — so a
   `tee` grant lets anything holding the rule append arbitrary content
   regardless of how exact the arguments look. Routing through these two
   helpers instead closes that gap: the validation lives in code the user
   can invoke but not edit, not in the sudoers pattern. (macOS has no
   NOPASSWD rule at all, so [`run_once_before_0001-install-zsh.sh.tmpl`](../run_once_before_0001-install-zsh.sh.tmpl)
   and [`run_once_after_0016-handoff-shell-to-brew.sh.tmpl`](../run_once_after_0016-handoff-shell-to-brew.sh.tmpl)
   fall back to a plain `sudo tee -a /etc/shells` there, prompting
   interactively every time — the helpers only exist on Linux.)

   A first-ever apply needs sudo in seven separate scripts — zsh (step 1),
   Homebrew's apt prerequisites (3), `unzip` for SDKMAN (7), pyenv's build
   deps (11), `socat` (15), registering the SSH agent relay's
   `SSH_AUTH_SOCK` in `/etc/environment` (also 15, for the systemd-based
   persistent relay), and `chsh` at the handoff (20) — and only this first
   script's own `visudo`/`install` calls (installing the drop-in itself)
   still prompt normally, since the rule they install doesn't exist yet to
   cover them. It prints a one-line explanation before that prompt rather
   ...
   A background timer sounds like the fix but doesn't hold up in practice:
   a systemd transient unit needs systemd actually running as PID 1, which
   isn't guaranteed on WSL2 (opt-in via `systemd=true` in `/etc/wsl.conf`;
   even item 15's own persistent SSH-relay service already treats systemd
   as best-effort rather than a hard dependency), and a transient unit
   doesn't survive a `wsl --shutdown`/reboot before it fires anyway. A
   plain detached background process has the same reboot-survival gap
   without even solving the "might not have systemd" problem. So instead,
   the backstop lives in
   [`.chezmoitemplates/sudo-timeout-backstop.sh.tmpl`](../.chezmoitemplates/sudo-timeout-backstop.sh.tmpl)
   (the private `_dotfiles_remove_stale_sudo_rule` function), included from
   two places:

   - [`dot_zshrc.tmpl`](../dot_zshrc.tmpl) — runs on every new interactive
     shell, right next to `dotfiles-login-reminder`, for as long as this
     file exists on disk at all.
   - This very script also writes that exact same function straight into
     `~/.zshrc` itself, in plain shell, bypassing chezmoi's own
     file-management step entirely, right after installing the sudoers
     rule. That's needed because `dot_zshrc.tmpl` only gets written to disk
     by a later, ordinary managed-file step in this same apply — if any
     `run_once_before_*` script after this one fails, chezmoi aborts before
     ever writing `~/.zshrc` for the first time, and the copy inside it
     would never even exist to run. Writing it here too means the backstop
     is in place from the instant the sudoers rule is, no matter what
     happens afterward; when chezmoi does get to manage `~/.zshrc` normally,
     it overwrites the whole file with `dot_zshrc.tmpl`'s content anyway
     (which includes the same function), so there's nothing to reconcile.

   Rather than guessing at how long a "stuck" apply looks like, the
   function tracks the actual `chezmoi apply` process: this script also
   records its own parent PID (chezmoi execs scripts directly, so `$PPID`
   *is* chezmoi), paired with that PID's process-start time (`/proc/<pid>`'s
   own mtime is set once, at process start), to
   `~/.cache/dotfiles-sudo-timeout.pid`. The backstop only removes the
   sudoers rule once no process matching *both* the recorded PID and start
   time is still alive — a bare PID-liveness check would eventually be
   fooled by an unrelated process reusing the same PID number long after
   chezmoi exits, quietly leaving `NOPASSWD` installed forever; pairing it
   with the start time rules that out. A `chezmoi apply` that's still
   genuinely running — a slow Homebrew or pyenv build, say — is never
   interrupted no matter how long it takes; one that's finished or crashed
   gets cleaned up on the very next shell, with no arbitrary wait at all. No
   daemon, no timer, nothing that can be lost across a reboot, and no way
   for scheduling the cleanup to itself abort the apply.
...
4. **Brew packages** — installed from the plain-text list in
   ...
   **JetBrains Junie CLI (macOS via brew, Linux via install.sh).** Listed in
   `brew-packages.txt` as `jetbrains/junie/junie`, but that formula's `url`
   has no `on_linux` branch at all — it unconditionally points at a macOS
   build, so `brew install` on Linuxbrew/WSL2 downloads a Mach-O binary that
   fails with "cannot execute binary file: Exec format error" (an upstream
   tap bug, not a Linuxbrew/WSL2 quirk).
   [`run_onchange_0004-brew-packages.sh.tmpl`](../run_onchange_0004-brew-packages.sh.tmpl)
   skips this one formula on Linux, and
...
5. **Terminal prompt (starship)** — `starship` (installed via
   ...
   **Native Windows PowerShell (WSL only)** — the starship binary above only
   ...
   [`run_once_0019-configure-powershell-profile.sh.tmpl`](../run_once_0019-configure-powershell-profile.sh.tmpl)
...
6. **SDKMAN!** — installed via the official curl installer, skipped if a
   ...
   the exact thing that manifest exists to enforce. `update-all` (item 25)
...
14. **Docker images** — pulled from
    ...
    install on both macOS and Windows — so this step skips quietly (always
    exits 0) if the `docker` CLI or daemon isn't reachable yet, or if
    there's no AWS session for any ECR-hosted images, or if an individual
    pull/login fails. Deliberately never fails the script for any of these —
    Docker not being installed yet and no `aws-login` session are the
    *expected* state on a fresh machine, not transient errors, and chezmoi
    aborts the entire rest of the apply (every later script, including the
    1Password relay, aws-vault, git clones, and the sudo cleanup step) the
    moment any one script exits non-zero. So there's no automatic retry for
    this script specifically — on a genuinely fresh machine, install Docker
    Desktop (enable WSL2 integration if on Windows) and/or run `aws-login
    <profile>`, then run `chezmoi apply` again by hand to trigger the pulls.
    Keep this list to
...
15. **1Password SSH agent relay (WSL only)** — installs `socat` (apt) and
    ...
    **A persistent copy of the relay, for anything that never sources
    `~/.zshrc` (WSL only).** The snippet above only starts the relay from an
    interactive zsh shell (or a `run_*` script that includes it directly) —
    it's invisible to anything that reaches WSL a different way. IDE git
    integrations are exactly that case: IntelliJ's WSL support, for example,
    invokes `/usr/bin/git` directly via `wsl.exe`, with no shell rc involved
    at all, so `SSH_AUTH_SOCK` was unset and the relay was never started even
    though a plain terminal in the same WSL instance worked fine.
    [`dot_config/systemd/user/ssh-agent-relay.service`](../dot_config/systemd/user/ssh-agent-relay.service)
    (excluded on macOS/bare-metal Linux via `.chezmoiignore`, same WSL2 guard
    as everywhere else in this step) runs the identical socat/npiperelay
    command as a `systemd --user` unit instead, and
    [`run_onchange_0020-enable-ssh-agent-relay-service.sh.tmpl`](../run_onchange_0020-enable-ssh-agent-relay-service.sh.tmpl)
    enables it — re-running automatically if the unit file's content ever
    changes, same manifest/`run_onchange_` pairing as every other step here,
    just with the unit file itself standing in for the manifest.

    Two things make this reach processes a shell-only fix can't:
    - `loginctl enable-linger <user>` keeps the user's systemd manager (and
      this service with it) running from boot, even if nothing has ever
      logged in interactively — otherwise `systemd --user` normally only
      starts on a real login session, which `wsl.exe` invoked straight from
      an IDE on the Windows side never creates.
    - `SSH_AUTH_SOCK` is additionally written to `/etc/environment` (via the
      new `tee -a /etc/environment` NOPASSWD rule in step 0 above) —
      system-wide, so it's visible to any process, not just ones that source
      `~/.zshrc`. `/etc/environment` has no variable expansion of its own,
      so this is the literal resolved path, not `$HOME`-relative.

    A new IDE terminal/process only picks up an `/etc/environment` change on
    its *next* fresh launch — same "close the window, don't just re-run"
    caveat as the Windows-side env vars in step 17 below, just on the WSL
    side this time. Check it's actually active with:
    ```bash
    systemctl --user status ssh-agent-relay.service
    grep SSH_AUTH_SOCK /etc/environment
    ```

    **This turned out not to be enough for IntelliJ specifically.** Testing
    showed `wsl.exe -d <distro> -e git ...` — IntelliJ's actual invocation —
    execs straight into `git` rather than going through a login shell, so it
    never runs PAM and never reads `/etc/environment` either: `wsl.exe -e
    printenv SSH_AUTH_SOCK` came back empty even with this relay running and
    `/etc/environment` correctly set (`wsl.exe -e printenv HOME` returned
    fine, so it's specifically an env-propagation gap, not a broken `wsl.exe`
...
18. **Env vars from 1Password (`op-login`)** — a generic version of
    ...
    **`gh` (GitHub CLI) via `GH_TOKEN`.** Unlike the tools above, this isn't
    a case this repo has to wire up specially — the `gh` CLI itself reads
    `GH_TOKEN` straight from the environment and uses it in place of its own
    stored login, so mapping it in `op-env-vars.txt` (needs a personal
    access token with `repo` and `read:org` scopes) is enough to
    authenticate `gh` non-interactively once `op-login` has run. Without it,
    authenticating `gh` is entirely on you — nothing here runs or checks it.
...
22. **Aliases** — a small "Aliases" block at the bottom of `dot_zshrc.tmpl`:
    ...
    - `_rsync-to-dev <host> <dest> <owner> <src files/globs...>` — shared
      helper behind every `*-dev-copy-qe-jars`/`*-dev-copy-qe-*` function
      below (same "private helper, `_`-prefixed, not listed in `help`"
      pattern as `_git_all_projects`). The `ubuntu` identity used for every
      server here can't write into another user's home directory, so rsync
      itself runs as root on the remote end (`--rsync-path="sudo rsync"`)
      rather than staging into a scratch dir and moving it into place
      afterwards — a scratch dir wiped clean on every run would give rsync
      nothing to diff against, re-sending everything on every run. Running
      rsync directly against the real destination lets it skip unchanged
      files and, via `--delete`, remove files no longer part of the local
      source (safe here since every destination this is used for holds only
      files this function itself manages, nothing else). The local source
      globs are symlinked into one local staging dir first — `--copy-links`
      makes rsync follow them and transfer the real content, directories
      included — because rsync's `--delete` only prunes extraneous files
      when syncing a whole directory, not an explicit list of individual
      files/globs. The rsync (and the underlying ssh connection) uses an SSH
      `ControlMaster`/`ControlPersist` connection, because three separate
      ssh/rsync steps back to back (an earlier version of the jars function)
      ...
      *local* staging dir's own owner/mode onto the destination directory —
      `mktemp -d`'s `0700` ended up applied to the destination directory
      itself, locked to whatever local user's numeric UID happened to map to
      on the server, breaking the target user's own access to it. A `sudo
      chown -R <owner>:<owner>` + recursive `find -exec chmod` (775 for
      directories, 664 for files) immediately after the rsync restores
      sane, consistent ownership/modes.
    - `bi-dev-copy-qe-jars`/`qs-dev-copy-qe-jars` — copy freshly built
      `target/*.jar` and `target/dependency/*.jar` from
      `~/projects/(bi|qs)-quote-engine` to that system's `*-app-dev` server's
      `/home/dev-java-engine-user/tomcat/lib`, via `_rsync-to-dev`.
    - `bi-dev-copy-qe-properties`/`qs-dev-copy-qe-properties` — copy that
      system's dev Tomcat config from
      `~/projects/sysadmin-tools/tomcat/application-configs/(bedfords|quotesearcher)-dev`
      to that system's `*-app-dev` server's `/home/dev-java-engine-user/gtconf`,
      via `_rsync-to-dev`. `bedfords`/`quotesearcher` are
    ...
    - `bi-dev-copy-qe-web`/`qs-dev-copy-qe-web` — copy
      ...
      user (and so the destination path and post-sync `chown` target)
...
23. **Git config** — [`dot_gitconfig.tmpl`](../dot_gitconfig.tmpl) →
    ...
    **`core.sshCommand`** routes every ssh git invokes through
    [`dot_local/bin/executable_git-ssh-with-agent.tmpl`](../dot_local/bin/executable_git-ssh-with-agent.tmpl)
    (→ `~/.local/bin/git-ssh-with-agent`) instead of calling `ssh` directly.
    This is the fix that actually solves an IDE's git integration (IntelliJ,
    VS Code) getting `Permission denied (publickey)` on WSL — step 15's
    `systemd --user` relay plus `/etc/environment` covers everything that
    inherits environment normally, but IntelliJ's WSL support invokes git via
    `wsl.exe -d <distro> -e git ...`, which execs straight into `git` rather
    than going through a login shell. That skips PAM entirely, so neither
    `~/.zshrc`'s export nor `/etc/environment` ever reaches that process —
    confirmed empirically: `wsl.exe -e printenv SSH_AUTH_SOCK` came back
    empty even with the relay active and `/etc/environment` correctly set,
    while `wsl.exe -e printenv HOME` returned correctly, isolating the gap to
    environment propagation specifically. `core.sshCommand` sidesteps the
    whole problem: git always calls the wrapper, and the wrapper sets
    `SSH_AUTH_SOCK` itself — via the same
    [`.chezmoitemplates/ssh-agent-sock.sh.tmpl`](../.chezmoitemplates/ssh-agent-sock.sh.tmpl)
    snippet steps 15/24 use — then `exec ssh "$@"`, so it works no matter how
    git itself was launched, without depending on any inherited environment
    at all. Verified by replicating IntelliJ's exact invocation from Windows:
    ```bash
...
24. **Clone git repos** —
    ...
    depend on the `gh` CLI being authenticated at all, and wouldn't even
    apply to a non-GitHub host anyway. SSH auth goes through the 1Password SSH agent
    ...
    within the loop (`|| echo "Failed to clone ... — skipping" >&2`,
    matching the other manifest-driven scripts): with the agent already
    proven working, one failure means that one repo (access not granted,
    renamed, deleted) and shouldn't abort the apply, and so every later step,
    on its way past. `git clone` cleans up its own partial directory on
    failure, so a skipped repo doesn't leave a stub that the "already
    cloned" check would later mistake for a real checkout. The script does
    still exit non-zero at the very end if any clone failed, though, so
    chezmoi retries it on the next apply instead of that repo staying
    silently uncloned until `git-repos.txt` itself happens to change again.
...
25. **`update-all`** — a `dot_zshrc.tmpl` function that updates everything
...
27. **Remove the bootstrap sudo NOPASSWD rule (Linux only)** —
    ...
    Runs last (`run_once_after`), after every script that needs the
    NOPASSWD rule has already run, so there's no reason to keep passwordless
    root access installed on the machine indefinitely. Since it's
    `run_once_after`, it only runs once per machine — a later
    `chezmoi state delete-bucket --bucket=scriptState` that forces an
    earlier script to rerun by hand will hit a normal interactive sudo
    prompt instead of NOPASSWD, which is fine for that kind of deliberate
    one-off rerun.

    This is the happy-path removal only — it depends on every script before
    it in the apply having succeeded, since chezmoi stops the whole apply on
    the first failing script. The `_dotfiles_remove_stale_sudo_rule`
